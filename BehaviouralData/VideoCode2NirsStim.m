% VideoCode2NirsStim(codeScheme, codingName, snirfDir, nirsDir)
...
% get info about which subject we'll working on
 [a, s, d] = fileparts(codingName);
 % remove cam info from sub name
 subName = strrep(s,'_cam1',[]);
 subName = strrep(s,'_cam2',[]);
 clear a s d
...
for a = 1:length(actions)
    ...
    lstIndx = ~cellfun(@isempty,lst);
    % name the condition
    codedData(lstIndx,1)=codeScheme(trig==1,3);
end

clear lst lstIndx trig a 
...
% find the corresponding snirf file
for iSub = 1:length(snirfDir)
    
   if  ~isempty(strfind(snirfDir(iSub).name,subName))
       snirfName = [snirfDir(iSub).folder filesep snirfDir(iSub).name];
   end
...
for iSub = 1:length(nirsDir)
    
   if  ~isempty(strfind(nirsDir(iSub).name,subName))
       nirsName = [nirsDir(iSub).folder filesep nirsDir(iSub).name];
   end
