...
% select only object exploration variables
c = [1,3,5];
codeScheme = codeScheme(c,:);

% change colours to match R plots (FITNG_23)
codeScheme{1,3}='#983430';
codeScheme{2,3}='#932DE7';
codeScheme{3,3}='#F1A93B';
...
for iSub = 1:size(filelist,1) % for each subject
    ...
    saveas(figure(iSub),[a filesep 'Plots/' subName '_VisBeh_FITNG23_NoLabels.png']);
