...
% Updated May 2023, Dr Ola Dopierala
% adapted for analyses planned for Exploration Pilot Paper: infant-directed
% speech and infant looking, blocks (>8s) and events (3-5s) 
%-----------------------------------------------------------------------
...
for iSub = 1%:size(filelist,1) % for each subject
    ...
    % cam1 always starts before cam2
    %--------
   ...
   for r = 1:size(actions,1) % search the annotated actions
    ...
    nirs.Conditions = [];
    nirs.Conditions{1,1}='Cam1_Start'; %onset of the recording camera 1
    nirs.Conditions{1,2}='Cam2_Start'; %onset of the recording camera 2 
    nirs.Conditions{1,3}=[beh1 '_Block']; %onsets of the behaviour 1 block
    nirs.Conditions{1,4}=[beh2 '_Block']; %onsets of the behaviour 2 block
    nirs.Conditions{1,5}=[beh1 '_Event']; %onsets of the behaviour 1 event
    nirs.Conditions{1,6}=[beh2 '_Event']; %onsets of the behaviour 2 event

    % Define the new file name
    newFileName = convertCharsToStrings([z filesep 'Coded' filesep 'EXPLPilot' filesep x '_Coded_' beh1 '_' beh2 '_' date v]);
   
    % Check if the parent directory exists
    if ~isfolder([z filesep 'Coded' filesep 'EXPLPilot'])
        % Create the parent directory if it doesn't exist
        mkdir([z filesep 'Coded' filesep 'EXPLPilot']);
    end
