%% Exploration project with LUMO (9.12.2022)
% Script to record synchronised video (2 cameras) and fNIRS data (LUMO)
% Author - Ola Dopierala, Baby Learning Lab, UBC
...
% a session of free play, we use LUMO system, and two logitech
...
% 3. starts cam1 camera and sends trigger to LUMO when recording has started
...
script_version='recordCameras_LUMO.m';
...
% "/dev/cu.usbserial-FTC1JK9U" with a default baud rate of 9600.
device = serialport("/dev/cu.usbserial-FTC1JK9U",9600);
...
[y1, fs1] = audioread('/Users/werkerlab/Desktop/EXPL_scripts/sound1.wav');
[y2, fs2] = audioread('/Users/werkerlab/Desktop/EXPL_scripts/sound2.wav');
...
% Write the data "cam1" as uint8 using the serialport object serialportObj.
write(device,"cam1","uint8"); %will show up as '1' in nirs
...
% Write the data "cam2" as uint8 using the serialport object serialportObj.
write(device,"cam2","uint8"); %will show up as '2' in nirs
...
%cam2
videoName2 = convertStringsToChars("/Users/werkerlab/Documents/EXPLORATION/recordings/EXPL"+ID+"_cam2_"+datestr(now,'yy.mm.dd')+"_"+datestr(now,'HH.MM')+".avi");
